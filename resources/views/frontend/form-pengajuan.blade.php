        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const suratFields = {
                    ...
                    <div class="space-y-1.5">
                        <label class="block text-sm font-semibold text-neutral-800">
                            Anggota Keluarga yang Ikut dalam Surat (JSON)
                        </label>
                        <textarea name="anggota_keluarga" rows="3"
                            class="w-full rounded-xl border border-emerald-100 px-3 py-2.5 text-sm focus:ring-2 focus:ring-emerald-500/70 focus:border-emerald-500 outline-none font-mono text-xs"
                            placeholder='[{"nama":"Rusdi","nik":"1607011212780008"},{"nama":"Siti","nik":"1607012312850002"}]'></textarea>
                        <p class="text-[11px] text-neutral-500">Opsional. Format JSON array. Kosongkan jika hanya pemohon saja.</p>
                    </div>
                `,
                        pribadiConfig: {
                            showNIK: true,
                ...
                function showStep(step) {
                    ...
                    document.querySelectorAll('[data-step-indicator]').forEach(el => {
                        ...
                        if (indicatorStep === step) {
                            ...
                            el.querySelector('div').classList.add('bg-emerald-600', 'text-white');
                            el.querySelector('div').classList.remove('bg-white', 'text-emerald-700');
                        } else if (indicatorStep < step) {
                            el.classList.remove('opacity-60');
                            el.querySelector('div').classList.add('bg-emerald-100', 'text-emerald-700');
                            el.querySelector('div').classList.remove('bg-emerald-600', 'text-white',
                            'bg-white');
                        } else {
                            el.classList.add('opacity-60');
                            el.querySelector('div').classList.add('bg-white', 'text-emerald-700');
                            el.querySelector('div').classList.remove('bg-emerald-600', 'text-white',
                                'bg-emerald-100');
                        }
                    });
                ...
                document.querySelectorAll('[data-next]').forEach(btn => {
                    btn.addEventListener('click', function() {
                        if (currentStep === 1) {
                            ...
                            if (!jenisSurat) {
                                document.getElementById('error_step1').classList.remove('hidden');
                                return;
                            }
                            document.getElementById('error_step1').classList.add('hidden');
                            ...
                            if (config) {
                                ...
                                document.getElementById('field_nik').style.display = config
                                    .pribadiConfig.showNIK ? 'block' : 'none';
                                document.getElementById('field_ttl').style.display = config
                                    .pribadiConfig.showTTL ? 'block' : 'none';
                                document.getElementById('field_umur').style.display = config
                                    .pribadiConfig.showUmur ? 'block' : 'none';
                                document.getElementById('field_status_perkawinan').style.display =
                                    config.pribadiConfig.showStatusPerkawinan ? 'block' : 'none';
                                document.getElementById('field_agama').style.display = config
                                    .pribadiConfig.showAgama ? 'block' : 'none';
                                document.getElementById('field_pekerjaan').style.display = config
                                    .pribadiConfig.showPekerjaan ? 'block' : 'none';
                                ...
                                document.getElementById('tanggal_lahir').required = config.pribadiConfig
                                    .showTTL;
                                document.getElementById('umur').required = config.pribadiConfig
                                .showUmur;
                        ...
                        if (currentStep === 2) {
                            ...
                            if (config) {
                                document.getElementById('dynamic-fields').innerHTML = config.fields;
                            }
                        }
            ...
            `;
                ...
                `;
                        if (formData.get('anggota_keluarga')) {
                            html +=
                                `<p><span class="text-neutral-500">Anggota Keluarga:</span> <span class="font-medium">${formData.get('anggota_keluarga')}</span></p>`;
                        }
                    } else if (kode === 'SKD') {
            ...
            `;
                    ...
                    document.getElementById('confirmation-summary').innerHTML = html;
                }
